% Computes image where the pixel values come from evaluating the second 
% second derivative of a Gaussian at each pixel's distance from the given
% line. This is meant to produce an on-center, off-surround filter for
% line segments.
% 
% ARGUMENTS
%   p1    - First endpoint of line
%   p2    - Second endpoint of line
%   sigma - Standard deviation of Gaussian whose 2nd derivative is being
%           evaluated
% 
% OUTPUT
...
function [output, offsetx, offsety] = gaussian2ndDerivLine(p1, p2, sigma)
...
g = -m*(((d.^2/sigma^2)-1).*exp(-d.^2/(2*sigma^2)));
%m = 1 / (sqrt(2*pi)*sigma);
%g = m*exp(-d.^2/(2*sigma^2));

% Truncate function when distance > 4*sigma
dGreaterThanIndices    = find(d > 4*sigma);
g(dGreaterThanIndices) = 0.0;

output = g./sum(sum(abs(g)));
offsetx = -colOffset+1;
offsety = -rowOffset+1;